...
//            additionalParam = referenceId+"-"+additionalParam;
    ...
    private void submitDetails() {
        ...
        if (spinnerZone.getSelectedItemPosition() == 0)
            ...
            qrLocationPojo.setZoneId(Integer.valueOf(zoneMap.get(spinnerZone.getSelectedItem())));

        if (spinnerWard.getSelectedItemPosition() == 0)
            qrLocationPojo.setWardId(0);
        else
            qrLocationPojo.setWardId(Integer.valueOf(zoneWardMap.get(spinnerWard.getSelectedItem())));

        if (spinnerArea.getSelectedItemPosition() == 0)
            qrLocationPojo.setAreaId(0);
        else
            qrLocationPojo.setAreaId(Integer.valueOf(areaMap.get(spinnerArea.getSelectedItem())));
